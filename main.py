...
class Game:
    def __init__(self, stdscr: curses.window):
        ...
        self.main_menu = Menu(["START", "OPTIONS", "LEVELS", "QUIT"], content=TITLE, start_y=0, start_x=0, width=85, height=15)
        self.options_menu = Menu(["BACK", f"MUSIC {'ON' if self.music_mode else 'OFF'}"], start_x=10, start_y=9)
        self.game_menu = Menu(["GO", "BACK", "LEFT", "RIGHT", "INVENTORY", "QUIT"], width=30, height=15)
    ...
    def load_level(number):
        with open(f"{BASEDIR}/levels/l{number}.json", 'r') as f:
            return json.load(f)
    ...
    def game_loop(self):
        ...
        self.level = level
        game_time = -1
        self.maze = Maze(level["maze"]["rows"], level["maze"]["cols"])
        self.layer = self.maze.copy()
        self.config = self.maze.config
        self.player = Player(self.maze.get_object_position('S'))
        self.inventory_menu = Menu([f"{key.upper()}: {value}" for key, value in self.player.inventory.items()] + ["BACK"])
        self.stdscr.clear()
        self.stdscr.addstr(f"Level starting: {level['name']}\n\n")
        self.stdscr.getch()
        self.stdscr.clear()
        while self.game_on:  # step
            if self.maze.is_end(self.player.position):  # Win
    ...
    def use_map(self):
        if self.player.inventory["maps"] == 0:
            self.stdscr.addstr("You don't have any maps!\n")
            return
        self.player.inventory["maps"] -= 1
        height, width = self.stdscr.getmaxyx()
        x = width - 50
        y = 0
        ...
        while True:
            ...
            for y, row in enumerate(buffer):
                for x, cell in enumerate(row):
                    if cell in [self.config["start"], self.config["end"]]:
                        panel.addch(y + 1, x + 2, cell, curses.color_pair(1))  # noqa 800
                    else:
                        panel.addch(y + 1, x + 2, cell)  # noqa 800
    ...
    def use_compass(self):
        if self.player.inventory["compasses"] == 0:
            self.stdscr.addstr("You don't have any compasses!\n")
            return
    ...
    def use_scanner(self):
        if self.player.inventory["scanners"] == 0:
            self.stdscr.addstr("You don't have any scanners!\n")
            return
    ...
    def use_locator(self):
        if self.player.inventory["locators"] == 0:
            self.stdscr.addstr("You don't have any locators!\n")
            return
